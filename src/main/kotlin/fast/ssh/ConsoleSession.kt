...
interface ConsoleSession : Closeable {
    companion object {
        fun sshCommand(cmd: String, session: GenericSshSession): IConsoleProcess
            = SshjProcessMom.newCommand(cmd, session)

        fun command(cmd: String, folder: File? = null): IConsoleProcess
            = NativeProcessMom.command(cmd.split(" ").toList(), folder)
    }
...
fun ConsoleSession.plainCmd(cmd: String, folder: String? = null) : IConsoleProcess {
    return when (this) {
        is GenericSshSession -> {
            val line =
                if (folder == null)
                    cmd
                else
            ...
            ConsoleSession.sshCommand(line, this)
        }
        is NativeSession -> {
            ConsoleSession.command(cmd, if(folder == null) null else File(folder))
        }
        else -> {
            TODO()
        }
...
suspend fun <T> ConsoleSession.runAndWait(
    ...
    = ProcessConsoleCommand(plainCmd(cmd), process, processErrors).runBlocking(timeoutMs)
